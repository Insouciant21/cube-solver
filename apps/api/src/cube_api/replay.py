...
def _apply(stickers: Mapping[str, Sequence[int]], order: int, token: str) -> StickerMap:
    ...
    axis, sign = {
        "U": ("y", -1),
        "D": ("y", 1),
        # The front/back sticker coordinate frame has its screen Y axis
        # inverted relative to the Three.js viewport. Keep replay aligned with
        # the visual turn so F/F' finishes on the animated state.
        "F": ("z", 1),
        "B": ("z", -1),
        "L": ("x", 1),
        "R": ("x", -1),
